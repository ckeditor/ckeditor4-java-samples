...
/**
 * The {@code DispatchServlet} class provides configuration for CKEditor
 * instances and is responsible for navigation to appropriate JavaServer Pages.
 */
public class DispatchServlet extends HttpServlet {
	...
	protected void doGet( HttpServletRequest request,
		...
		if ( requestPath.endsWith( EXTENSION ) ) {
			...
			String requestFile = jspPath
					.substring( jspPath.lastIndexOf( "/" ) + 1 );
	...
	/**
	 * Puts appropriate configuration options, based on file name, in scope so
	 * that they are available for JavaServer Pages to use.
	 * 
	 * @param requestFile
	 *            a string representing the JSP file name for which the configuration
	 *            should be provided.
	 */
	private void provideConfiguration( String requestFile,
			HttpServletRequest request ) {
		if ( requestFile.equals( "replaceTag.jsp" ) ) {
			request.setAttribute( "cke_config",
					ConfigurationProvider.getFirstEditorConfig( ) );

		} else if ( requestFile.equals( "replaceAllTag.jsp" ) ) {
			request.setAttribute( "cke_config",
					ConfigurationProvider.getSecondEditorConfig( ) );

		} else if ( requestFile.equals( "insertTag.jsp" ) ) {
			request.setAttribute( "cke_config",
					ConfigurationProvider.getThirdEditorConfig( ) );
			request.setAttribute( "cke_textarea_attrs",
					ConfigurationProvider.getTextareaAttrs( ) );

		} else if ( requestFile.equals( "insertInlineTag.jsp" ) ) {
			request.setAttribute( "cke_init_value",
					ConfigurationProvider.getInitValue( ) );
			...
			request.setAttribute( "cke_global_events",
					ConfigurationProvider.getGlobalEvent( ) );

		} else if ( requestFile.equals( "timestamp.jsp" ) ) {
			request.setAttribute( "cke_timestamp",
					( ( int ) ( Math.random( ) * 50 + 1 ) ) );

		} else if ( requestFile.equals( "globalConfig.jsp" ) ) {
			request.setAttribute( "ckeditor_global_config",
					ConfigurationProvider.getFourthEditorConfig( ) );
